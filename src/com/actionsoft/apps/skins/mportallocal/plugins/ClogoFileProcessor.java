...
public class ClogoFileProcessor extends AbstFileProcessor implements FileProcessorListener {

	private static final String[] images = { "png", "jpg", "jpeg" };
	...
	public boolean uploadReady(Map<String, Object> param) {
		...
		context.setFileName("tempclogo.jpg");
		// 保存源图片
		DataInputStream data = (DataInputStream) param.get("data");
		DataOutputStream os = null;
		File tempImage = new File(context.getPath() + context.getFileName()); // 临时文件名
		try {
	...
	public InputStream downloadContent(Map<String, Object> param) throws Exception {
		DCContext context = (DCContext) param.get("DCContext");
		InputStream clogo = new FileInputStream(context.getFilePath());
		return clogo;
	}
